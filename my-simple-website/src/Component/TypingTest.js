...
const TypingTest = () =>{
    ...
    const givenTime = 30;
    const [timer,setTimer] = useState(givenTime);
    const [typingQuest,setTypingQuest] = useState('도토리는 맛있다.') //현재 목표
    const [startTime,setStartTime] = useState();
    const [endTime,setEndTime] = useState();
   // const [currentQuest,setCurrentQuest] = useState(''); //currentQuest 는 재시작 용 임시저장 변수
    const restart = () =>{
        setTextInput('');
        setIsAnswer(false);
        setIsRestart(false);
        //setCurrentQuest(typingQuest);
        //setTypingQuest(currentQuest);
        setTimer(givenTime);
    }
    ...
    const toNextStage=()=>{
        ...
        setIsRestart(false);
        setTypingQuest(()=>{
            const index = Math.floor(Math.random() * lifeQuotes.length);
    ...
    return(
        <div className="typing-test">
            ...
            <div className="typing-container">
                <textarea
                value ={textInput}
                onChange={valueChangeEventHandler}
                disabled={isAnswer||isRestart}
                />
            </div>
            <img src="/hancom.jpg" className="character-image"/>
            <p>마감까지 남은 시간 : {timer}</p>
            {isAnswer&&
            <div>
            <Result inputText={textInput} answerText={typingQuest}/>
            <p> 걸린 시간 : {(endTime - startTime)/1000.00} 초</p>
            <button onClick={toNextStage}>다음 단계로 이동하기</button>
            </div>
            }
            {isRestart && <button onClick={restart}>다시 시작하기</button>}
        </div>
    )
}
...
"인생은 오늘의 내 안에 있고 내일은 스스로 만드는것이다. -L론허바드",
